...
impl Grid {
    ...
    pub fn from_file(mut file_name: &str) -> Option<Self> {
        ...
        if file_name.ends_with(".gz") {
            ...
            if !output.stderr.is_empty() {
                println!(
                    "Error encountered decompressing file {}: {:?}",
                    file_name,
        ...
        if read_result.is_err() {
            println!(
                "Error encountered reading from filesystem: {:?}",
                read_result
            );
            return None;
        }
    ...
    pub fn from(grid_type: GridType, width: u32, height: u32) -> Self {
        ...
        let cells = match grid_type {
            ...
            GridType::Circle => {
                Self::cells_circle(width, height, std::cmp::min(width, height) / 10)
            }
            // default to center
    ...
    fn cells_empty(width: usize, height: usize) -> Vec<bool> {
        ...
        let size: usize = width
            .checked_mul(height)
            .expect("Grid size should fit in a usize!");
    ...
    fn cells_center(width: usize, height: usize) -> Vec<bool> {
        ...
        let mid_idx = width * height / 2 + (width / 2);
        new[mid_idx] = true;
        new
    }
    ...
    fn cells_circle(width: usize, height: usize, radius: usize) -> Vec<bool> {
        let mut new = Self::cells_empty(width, height);
        let midx = width / 2_usize;
        let midy = height / 2_usize;
